...
@section('content')
@php
    $is_mobile = preg_match('/(android|iphone|ipod|ipad|windows phone|blackberry|mobile)/i', request()->userAgent());
    $hasvideo = false;
...
<section class="hero-content">
    <div>
        <div id="heroSlider" class="hero-slider carousel slide carousel-fade" data-ride="carousel" data-interval="{{ $hasvideo ? '6000' : '5000' }}">
            ...
            <div class="carousel-inner">
                @foreach($homegallery as $index => $item)
                    @php
                        ...
                        if ($locale == 'ar') {
                            $photo = $is_mobile && !empty($item->photo_mobile_ar) ? $item->photo_mobile_ar : ($item->photo_ar ?? $photo);
                        }
                    ...
                    @if(!empty($item->videourl))
                        <div class="item {{ $active }} full">
                            <div class="carousel-video">
                                <video class="slider-video" width="100%" autoplay loop muted preload="auto" playsinline style="visibility: visible; width: 100%" poster="{{ $imageUrl }}storage/{{ $item->photo }}">
                                    <source src="{{ $imageUrl }}storage/{{ $item->videourl }}" type="video/mp4">
                                </video>
                            </div>
                        </div>
                    @else
                        <div class="item {{ $active }}">
                            <a href="{{ !empty($item->link) ? $item->link : '#' }}">
                                <img src="{{ $imageUrl }}storage/{{ $photo }}" alt="{{ $title }}" class="img-responsive fill2">
                            </a>
                        </div>
                    @endif
                @endforeach
                    ...
                    <div class="product-box">
                        <a href="{{ route('product.show', ['locale' => $locale, 'category' => $product->categorycode, 'product' => $product->productcode]) }}">
                            <div class="product-image">
                                <img src="{{ \App\Helpers\ImageHelper::url($photo, 'thumb-') }}" alt="{{ $productTitle }}" class="img-responsive">
                                @if($discount > 0)
                            ...
                            <div class="product-info">
                                ...
                                <div class="product-price">
                                    @if($discount > 0)
                                        <span class="old-price">{{ number_format($price * $currencyRate, 3) }} {{ $currencyCode }}</span>
                                        <span class="new-price">{{ number_format($finalPrice, 3) }} {{ $currencyCode }}</span>
                                    @else
                                        <span class="price">{{ number_format($finalPrice, 3) }} {{ $currencyCode }}</span>
                                    @endif
                                </div>
                            </div>
                        </a>
