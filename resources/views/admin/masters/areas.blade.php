            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="areasTable" class="table table-bordered table-striped table-hover" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Areas Name</th>
                                    <th>Areas Name (AR)</th>
...
@section('scripts')
    <script>
        (function() {
            ...
            function initAreasDataTable() {
                loadDataTables(function() {
                    if (typeof jQuery === 'undefined' || typeof jQuery.fn.DataTable === 'undefined') {
                        setTimeout(initAreasDataTable, 50);
                        return;
                    }

                    const $ = jQuery;
                    const areaBaseUrl = '{{ url("/admin/areas") }}';
                    ...
                    $(document).ready(function() {
                        ...
                        function showLoader() {
                            if (!loadingModal) {
                                ...
                                const modalEl = document.getElementById('areaDataTableLoader');
                            ...
                            columns: [{
                                ...
                                },
                                {
                                    ...
                                    render: function(data, type, row) {
                                        ...
                                        html +=
                                            '<a href="javascript:void(0);" class="btn btn-light btn-icon btn-sm rounded-circle view-area-btn" data-area-id="' +
                                            row.action + '" title="View">';
                                        html += '<i class="ti ti-eye fs-lg"></i></a>';
                                        @unless(\App\Helpers\ViewHelper::isView('areamaster'))
                                        html +=
                                            '<a href="javascript:void(0);" class="btn btn-light btn-icon btn-sm rounded-circle edit-area-btn" data-area-id="' +
                                            row.action + '" title="Edit">';
                                        html += '<i class="ti ti-edit fs-lg"></i></a>';
                                        html +=
                                            '<a href="javascript:void(0);" class="btn btn-light btn-icon btn-sm rounded-circle delete-area-btn" data-area-id="' +
                                            row.action + '" data-area-name="' + (row.areaname || 'this area') +
                                            '" title="Delete">';
                        ...
                        $(document).on('click', '.delete-area-btn', function(e) {
                            e.preventDefault();
                            deleteAreaId = $(this).data('area-id');
                            const areaName = $(this).data('area-name') || 'this area';
                            $('#deleteAreaName').text(areaName);
                            const deleteModal = new bootstrap.Modal(document.getElementById('deleteAreaModal'));
                            deleteModal.show();
                        });
                        ...
                        $('#confirmDeleteAreaBtn').on('click', function() {
                            if (deleteAreaId) {
                                ...
                                AdminAjax.request(areaBaseUrl + '/' + deleteAreaId, 'DELETE')
                                    .then(res => {
                        ...
                        function openAreaFormModal(areaId = null) {
                            cleanupModals();
                            const url = areaId ? areaBaseUrl + '/' + areaId + '/edit' : areaBaseUrl + '/create';
                        ...
                        function submitAreaForm(form, areaId, modal) {
                            ...
                            AdminAjax.request(url, method, formData)
                                .then(res => {
                                    showToast(res.message || 'Area saved successfully', 'success');
                                    setTimeout(() => {
                                        modal.hide();
                                    }, 1500);
                                    showLoader();
                        ...
                        function cleanupModals() {
                            ...
                            $('#areaModal').remove();
                            $('#areaViewModal').remove();
                        }

                        function loaderHtml() {
                            return `
                                <div class="modal fade" id="areaDataTableLoader" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-body text-center p-4">
                                                ...
                                                <h5 class="mt-3 mb-1">Loading Areas...</h5>
