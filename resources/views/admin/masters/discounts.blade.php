            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="discountsTable" class="table table-bordered table-striped table-hover" style="width:100%">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Discount Rate (%)</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete discount <strong id="deleteDiscountRate"></strong>?</p>
...
@section('scripts')
    <script>
        (function() {
            ...
            function initDiscountsDataTable() {
                loadDataTables(function() {
                    if (typeof jQuery === 'undefined' || typeof jQuery.fn.DataTable === 'undefined') {
                        setTimeout(initDiscountsDataTable, 50);
                        return;
                    }

                    const $ = jQuery;
                    const discountBaseUrl = '{{ url("/admin/discounts") }}';
                    ...
                    $(document).ready(function() {
                        ...
                        function showLoader() {
                            if (!loadingModal) {
                                ...
                                const modalEl = document.getElementById('discountDataTableLoader');
                        ...
                        function cleanupLoader() {
                            $('#discountDataTableLoader').remove();
                        ...
                        let table = $('#discountsTable').DataTable({
                            processing: true,
                            serverSide: true,
                            dom: 'rtip',
                            ajax: {
                            ...
                            columns: [{
                                ...
                                },
                                {
                                    ...
                                    render: function(data, type, row) {
                                        ...
                                        html +=
                                            '<a href="javascript:void(0);" class="btn btn-light btn-icon btn-sm rounded-circle view-discount-btn" data-discount-id="' +
                                            row.action + '" title="View">';
                                        html += '<i class="ti ti-eye fs-lg"></i></a>';
                                        html +=
                                            '<a href="javascript:void(0);" class="btn btn-light btn-icon btn-sm rounded-circle edit-discount-btn" data-discount-id="' +
                                            row.action + '" title="Edit">';
                                        html += '<i class="ti ti-edit fs-lg"></i></a>';
                                        html +=
                                            '<a href="javascript:void(0);" class="btn btn-light btn-icon btn-sm rounded-circle delete-discount-btn" data-discount-id="' +
                                            row.action + '" data-discount-rate="' + (row.rate || 'this discount') +
                                            '" title="Delete">';
                        ...
                        function openDiscountViewModal(discountId) {
                            cleanupModals();
                            const url = discountBaseUrl + '/' + discountId;
                            ...
                            AdminAjax.get(url).then(response => {
                                ...
                                loadingModal.hide();
                                cleanupModals();
                                AdminAjax.showError('Failed to load discount details.');
                            });
                        }

                        function openDiscountFormModal(discountId = null) {
                            cleanupModals();
                            const url = discountId ? discountBaseUrl + '/' + discountId + '/edit' : discountBaseUrl + '/create';
                        ...
                        function submitDiscountForm(form, discountId, modal) {
                            ...
                            AdminAjax.request(url, method, formData)
                                .then(res => {
                                    showToast(res.message || 'Discount saved successfully', 'success');
                                    setTimeout(() => {
                                        modal.hide();
                                    }, 1500);
                                    showLoader();
