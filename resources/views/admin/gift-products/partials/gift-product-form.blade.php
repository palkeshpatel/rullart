<div class="modal fade" id="giftProductModal" tabindex="-1" aria-labelledby="giftProductModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="giftProductModalLabel">
                    {{ $product ? 'Edit Gift Product' : 'Add Gift Product' }}
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="giftProductForm" method="POST"
                action="{{ $product ? route('admin.gift-products.update', $product->productid) : route('admin.gift-products.store') }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="photo1" id="photo1Input" class="form-control"
                                    accept="image/*" onchange="previewImage(this, 'photo1Preview')">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="photo2" id="photo2Input" class="form-control"
                                    accept="image/*" onchange="previewImage(this, 'photo2Preview')">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="photo3" id="photo3Input" class="form-control"
                                    accept="image/*" onchange="previewImage(this, 'photo3Preview')">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#productCategory1').on('change', function() {
            loadSubcategories($(this).val(), 'subCategory1');
        });
        
        // Product Category 2
        $('#productCategory2').on('change', function() {
            loadSubcategories($(this).val(), 'subCategory2');
        });
        
        // Product Category 3
        $('#productCategory3').on('change', function() {
            loadSubcategories($(this).val(), 'subCategory3');
        });
        ...
        @if(isset($product) && $product)
            @if($product->productcategoryid)
                setTimeout(function() {
                    ...
                    // After subcategories load, select the one stored in productcategoryid2
                    setTimeout(function() {
            ...
            @endif
            @if($product->productcategoryid2 && !$product->productcategoryid)
                // If productcategoryid2 exists but productcategoryid doesn't, it might be a subcategory
                setTimeout(function() {
                    // Try to find parent category and load subcategories
                    // For now, just set productcategoryid2 as Product Category 2
                    $('#productCategory2').val({{ $product->productcategoryid2 }}).trigger('change');
                }, 200);
            @endif
            @if($product->productcategoryid3)
                setTimeout(function() {
                    $('#productCategory3').val({{ $product->productcategoryid3 }}).trigger('change');
                    // After subcategories load, select the one stored in productcategoryid4 (if exists)
                    setTimeout(function() {
                        @if($product->productcategoryid4)
                            $('#subCategory3').val({{ $product->productcategoryid4 }});
                        @endif
                    }, 500);
                }, 300);
            @endif
    ...
    (function() {
        function initGiftProductValidation() {
            if (typeof jQuery === 'undefined' || typeof jQuery.fn.validate === 'undefined') {
                setTimeout(initGiftProductValidation, 50);
                return;
            }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                if (typeof showToast === 'function') {
                                    showToast(response.message || 'Gift product saved successfully', 'success');
                                } else if (typeof toastr !== 'undefined') {
                                    toastr.success(response.message || 'Gift product saved successfully');
                                }
                                // Close modal and reload table
                                const modal = bootstrap.Modal.getInstance(document.getElementById('giftProductModal'));
                                if (modal) modal.hide();
                                if (typeof table !== 'undefined' && table) {
                                    table.ajax.reload();
                                } else {
                                    setTimeout(function() {
                                        window.location.href = response.redirect || '{{ route("admin.gift-products") }}';
                                    }, 1000);
                                }
                            } else {
                                if (typeof showToast === 'function') {
                                    showToast(response.message || 'Failed to save gift product', 'error');
                                } else if (typeof toastr !== 'undefined') {
                                    toastr.error(response.message || 'Failed to save gift product');
                                }
                        ...
                        error: function(xhr) {
                            ...
                            if (xhr.status === 422) {
                                ...
                                const message = xhr.responseJSON?.message || 'An error occurred while saving the gift product';
                                if (typeof showToast === 'function') {
                                    showToast(message, 'error');
                                } else if (typeof toastr !== 'undefined') {
                                    toastr.error(message);
        ...
        $(document).ready(function() {
            ...
            $('#giftProductModal').on('shown.bs.modal', function() {
                initGiftProductValidation();
            });
            
            // Also try to initialize immediately if modal is already open
            if ($('#giftProductModal').hasClass('show')) {
                initGiftProductValidation();
            } else {
                initGiftProductValidation();
            }
    ...
    (function() {
        ...
        $(document).on('click', '#confirmDeleteImageBtn', function() {
            ...
            // Make AJAX request
            $.ajax({
                url: '{{ route('admin.gift-products.remove-image', ':id') }}'.replace(':id',
                    productId),
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
