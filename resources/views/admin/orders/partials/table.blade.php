                ...
                <tr data-order-id="{{ $order->orderid }}" class="order-row">
                    <td>
                        <a href="javascript:void(0);"
                            class="text-primary toggle-order-details d-inline-flex align-items-center gap-2"
                            data-order-id="{{ $order->orderid }}" style="text-decoration: none;">
                    ...
                    <td>
                        ...
                        <select class="form-select form-select-sm order-status" data-order-id="{{ $order->orderid }}">
                            ...
                            <option value="4" {{ $order->fkorderstatus == 4 ? 'selected' : '' }}>Delivered</option>
                            <option value="5" {{ $order->fkorderstatus == 5 ? 'selected' : '' }}>Cancelled
                            </option>
                            <option value="6" {{ $order->fkorderstatus == 6 ? 'selected' : '' }}>Returned</option>
                            <option value="7" {{ $order->fkorderstatus == 7 ? 'selected' : '' }}>Shipped</option>
                        </select>
                    </td>
                    <td>{{ $order->country }}</td>
                    <td>{{ $order->orderdate ? \Carbon\Carbon::parse($order->orderdate)->format('d/M/Y H:i') : 'N/A' }}
                    </td>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-4 text-end">
                                    <div class="d-flex justify-content-end gap-1 align-items-center">
                                        ...
                                        <a href="javascript:void(0);"
                                            class="btn btn-light btn-icon btn-sm rounded-circle view-order-btn"
                                            data-order-id="{{ $order->orderid }}" title="View">
                                            <i class="ti ti-eye fs-lg"></i>
                                        </a>
                                        <a href="{{ route('admin.orders.edit', $order->orderid) }}"
